#!/usr/bin/env python3
"""Run matched GPU-union control versus batched new-K/V D2H variants."""
    ...
    if mode != "control":
        result.extend(
            [
                ...
                "--twilight-new-kv-d2h-granularity", mode,
            ]
        )
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results/twilight_batched_new_kv_d2h_v1/formal",
    )
...
def main() -> int:
    ...
    fingerprints = {str(path.relative_to(ROOT)): sha256(path) for path in SOURCE_FILES}
