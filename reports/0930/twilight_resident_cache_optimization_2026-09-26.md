...
### 最新 normal-overlap timeline：003 的三個 diagnostic tokens

另以相同 optimized cache 參數，先量同輪未插樁 D2–D32 TPOT **92.424 ms/token**，再於正式計時後量三個共同起點 CUDA Event／CPU wall timeline token。Diagnostic wall 分別 **96.947 / 99.118 / 99.817 ms**，平均 **98.627 ms**，比該輪正式 TPOT 高約 **6.71%**。以下百分比**只以 98.627 ms diagnostic wall 為分母**，不是正式 TPOT 的元件百分比。

| 共同起點非重疊大類 | 三 token 平均 | diagnostic wall 比例 | 邊界 |
|---|---:|---:|---|
| Selection exposed | 26.151 ms | 26.52% | Selection core CUDA intervals |
| Selection 完成 → Attention 開始 | 44.178 ms | 44.79% | 每 layer 以 Selection core end 到 Attention GPU start 為界 |
| 已標記 model compute active | 26.825 ms | 27.20% | QKV/RoPE prep 與 embedding、Attention、projection、MLP 等 CUDA intervals |
| 未歸屬 remainder | 1.473 ms | 1.49% | 由 diagnostic wall 扣除上述 intervals；不是特定函式的確定成本 |

上述三類在這三個 common-origin token 中沒有相互 overlap。其內部子項仍會重疊，**不可再加到大類上**。例如同輪 CPU gather active **4.115 ms**、selected-KV H2D active **4.382 ms**，交集 **1.736 ms**；各自未重疊部分為 **2.378 / 2.646 ms**。Selection 內的 GPU GQA union **1.159 ms**、index bitmap D2H **1.460 ms**、CPU bitmap decode **5.919 ms**也只是子項／active scope。另一個 D33 breakdown run 的 resident mapping、index H2D、hit copy 與 snapshot 表格在上方，**不是這三個 timeline token 的同次元件計時**。
...
- 三個 normal-overlap timeline tokens 的 wall 高於同輪正式 TPOT；其非重疊大類百分比只描述 diagnostic run，不可直接投射到 94.483 ms/token 的三題 formal mean。
...
- 最新 common-origin timeline：`results/twilight_resident_optimized_current_v1/normal_overlap/003_qa_1_i011/{result.json,timeline_token1.json,timeline_token2.json,timeline_token3.json}`；分析使用 `scripts/analyze_twilight_current_cost_profile_v1.py` 的 `calculate_token` interval 規則。
